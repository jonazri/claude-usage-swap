...
> **Annotation 2026-07-02 (superseded in per_session mode):** the "cannot run two accounts simultaneously" trade-off holds only for `mode: global`. `mode: per_session` (plan: `docs/plans/2026-07-02-per-session-accounts.md`) adds slot dirs — one `CLAUDE_CONFIG_DIR` mount per concurrent session, each holding one account's credentials, swapped in-place per slot. Global mode remains the default and this section stays accurate for it. See "Per-session slot dirs" below for the full config-dir inventory that replaces the "no profile dirs" simplification in that mode.
