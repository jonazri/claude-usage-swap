...
| new helpers | `_capacity_ctx(state, config)` → `{reference_x, capacity_x_by_name, lane_load_by_name}` (lane loads from live occupancy + this-cycle claims; committee round-2 — the round-1 payload couldn't evaluate formula 2); `_remaining_units(pct, name, ctx, config)` — takes `pct` explicitly: each call site passes the pct expression it computes today (per the pct source rule; G4's `max(raw, extrapolated)` exists only locally in `decide_swap`), the helper never computes pct itself |
| **plumbing** | `_target_would_immediately_re_trip` and `_launch_candidate_saturated` gain `(name=None, ctx=None)` params — `None` ⇒ today's percent path. `_target_would_immediately_re_trip`'s fleet-level callers (4679, 4700, 7750, 9808) iterate name-keyed collections and pass both. `_launch_candidate_saturated`'s body IS the launch-accept wall (2434–2441: the steps line via its internal re-trip call at 2436, the hard-7d reject at 2439–2441 → G9); its ONLY external caller is the fresh-reading verify-and-repick loop at 17368, which must pass name+ctx, reusing the launch ctx stashed by `pick_launch_account`'s shim builder (left on the percent path it would re-reject every gate-on-accepted target, e.g. idle 20x@80%, and churn the verify loop). Decision layers stash `state["_capacity_ctx"]` (the `state["_lane_load"]` shim precedent); `pick_launch_account`'s shim builder additionally builds lane occupancy from `occupied_slot_accounts(state)` (slot/mount counts — today's `_lane_load` semantics) |
| `pick_swap_target` (~4490) | G1 key; `_lane_load_penalty` returns 0 in gate-on mode; drain/strict_priority accepts converted (G7) |
| `decide_swap` (~6994, ~7183, ~7191, ~7381) | G9 Trigger-1 7d cap, G6 precheck, G4 trip, G5 hysteresis |
...
| SOS / status | G8 probes take ctx (target-side formula 2, source-side formula 3); G9 labeller (9804); stateless `reference_x`-vs-observed-min drift warning; sub-reference-account warning; `capacity_x` + units shown in `cus status` (display only) |
