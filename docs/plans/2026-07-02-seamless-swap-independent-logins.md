...
>
> **Not built:** Phase 2 (wire the store into the swap path behind the gate), Phase 3 (lift #104 refusal for independent families + make `classify_live_creds_owner`/save-back per-(mount,account) aware), Phase 4 (expiry management + docs + flip default). Line refs in §3/§8 below are from the pre-`main` revision; current touchpoints: `classify_live_creds_owner` ≈ L2779, the two #104 refusals in `switch` and `_launch_prepare`, swap install site in `_execute_swap_locked`.
