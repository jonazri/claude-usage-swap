...
5. **7d in ladder scope (#56):** `seven_day: false` today. Re-add as a "no real danger, don't churn" signal?
   ...
   > **RESOLVED 2026-06-12 — re-added (`seven_day: true`).** The "no real danger, don't churn" framing turned out to be the bug, not a feature: with the 7d window hidden from the ladder, the daemon swapped a session ONTO `default` at 7d=90% / 5h=0% on 2026-06-11T21:04 and parked it there ~6h (the ladder only watched the idle 5h window). Investigation (incl. two Fable audits) showed the churn this flag was meant to kill (06-08 ping-pong) was purely **5h**-driven at 7d≈12% — re-enabling 7d produces bit-identical decisions for that episode, and the churn is independently held by `min_improvement_pct: 5` + `lazy_swap`. Replaced the split-brain (only `hard_7d_cap` knew about 7d) with ONE rule: an account is "full" at ≥90% on **either** window. Changes: `thresholds.seven_day: true`; `hard_7d_cap_pct` demoted 80→95 (rare emergency backstop); and a code fix to `_target_would_immediately_re_trip` (cus.py ~951) clamping the "full" test to `steps[0]` so a parked account whose ladder ratcheted to 96 (5h=0% never rolls → never resets) is still seen as full. Shipped on `fix/7d-aware-swap-logic-20260612` + `tests/test_7d_aware_swap.py`. See that branch's PR.
