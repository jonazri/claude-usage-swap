...
| 11b | **Wake-up message as positional arg causes first-prompt round-trip + Stop hook fire** | `cus.py:1559` | `claude --resume <id> "Continue where you left off."` — the positional arg becomes a user message after resume. Claude responds (Opus, 5h-window tokens). Then user's `validator.sh` Stop hook fires, calls `claude -p --model haiku "<validator template>"` (Haiku, same 5h window). Cost per session: ~1% of 5h. × N bungled sessions = noticeable burn. **Fix**: when re-enabling hot-swap, drop the positional wake-up entirely. Relaunched session opens at empty prompt; user types whatever they want next. Cost: lose explicit "you can continue" cue, but no token round-trip. |
