...
| 9 | **Tier 3 single Escape** | `cus.py:1523` | ~~Single `tmux send-keys Escape` may not interrupt a running tool reliably; Claude docs suggest double-Esc. **Recommendation**: Esc, sleep 0.3, Esc again.~~ **WITHDRAWN 2026-08-04 — do NOT do this.** Measured against a live Claude Code 2.1.221 pane: two *delivered* Escapes are the TUI's double-esc **gesture**, which clears the user's draft (non-empty input) or opens the **Rewind** overlay (empty input) — where the `/exit`/Enter that follows can select a checkpoint and revert code. "Esc, sleep 0.3, Esc again" also keeps both bytes inside the keypress parser's 500 ms escape-code window, so the next byte sent is eaten as `Alt+<byte>` (this is what dropped the leading character of the reactive-resume message). One Escape plus a settle wait is correct — see `tmux_escape_prefix`. |
