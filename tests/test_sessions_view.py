...
def test_binding_names_the_allowlisted_model_not_the_highest_untracked_one():
    """With per_model_weekly.models as an allowlist, the label must name the
    model whose value drives the gate — not a disallowed model that happens to
    read higher (Copilot review of #224)."""
    cfg = cus.deep_merge(_config(), {"per_model_weekly": {"gate_enabled": True, "models": ["Fable"], "cap_pct": 95}})
    acct = _acct(five=10.0, per_model={"Sonnet": 99.0, "Fable": 96.0})
    sev, txt = cus._session_binding(acct, "premium", cfg)
