...
def test_real_guard_refuses_dead_legacy_store_at_launch():
    """The REAL guard, end to end: an empty slot whose (alpha, slot) legacy
    login store is dead (probe → invalid_grant) is refused at launch with a
    DeadLegacyStoreError (surfaced as a ClickException on the --lane path), and
    the dead store is never installed. The daemon swap path keeps its own
    fallback (a non-ok legacy verdict installs the guarded snapshot instead)."""
    env = _Env()
    try:
        ...
        with pytest.raises(cus.click.ClickException) as ei:
            cus._launch_prepare("alpha", cus.load_state(), cus.load_config(), lane=slot)
        msg = str(ei.value)
