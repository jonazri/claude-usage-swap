...
def test_probe_unknown_fails_open_and_launches():
    # The canonical shares the mount's generation: a STRICTLY fresher canonical
    # on a different refresh token is a superseded mount and reinstalls by design.
    env = _Env({"acct": _valid("at-canon", "rt-maybe"), "other": _valid("at-o", "rt-o")},
               active="other", config=_ILGATE)
    try:
